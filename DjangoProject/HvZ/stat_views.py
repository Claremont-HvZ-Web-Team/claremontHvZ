...
def get_stat(game, constraints):
	...
	baseQuery = "select count(*) from HvZ_Character, HvZ_player where HvZ_player.id=player_id and HvZ_registration.game_id="
...
def dorm_populations(dorms, game):
    ...
    for d in dorms.select_related("campus"):
		...
		'name':str(d),
		'id':str(d.id),
		'humans': get_stat(game, ["team='H'", "dorm_id="+str(d.id)]),
		'zombies': get_stat(game, ["team='Z'", "dorm_id="+str(d.id)])
		}
...
def stats_category_view(request,category):
	g = get_current_game()
	ui = get_user_info(request)
	all = Character.objects.filter(game=g)
	data = []
	if category=="school":

...
def get_on_duty():
    ...
    od_object = OnDuty.objects.filter(
...
class EatForm(forms.Form):
    ...
    HOURS = ((str(n), str(n)) for n in range(1, 13))

    MINS = (("%02d" % n, "%02d" % n) for n in range(0, 60)[::5])
    ...
    meal_day = forms.ChoiceField(
        choices=DAYS, initial=t.weekday(),
        widget=forms.RadioSelect, required=True,
        help_text="<div class='reminder'>Select the day that you ate them. </div>"
    )
    meal_hour = forms.ChoiceField(
        label="Meal Time", choices=HOURS,
        initial=cur_hour, required=True
    )
    meal_mins = forms.ChoiceField(
        label=" ", choices=MINS,
        initial=cur_min, required=True
    )
    meal_ap = forms.ChoiceField(
        label="", choices=AP,
        initial=cur_ap, required=True,
        help_text="<div class='reminder' >Select the time of the meal; if you can't remember just type in now.</div>"
    )
    location = forms.ModelChoiceField(
        queryset=Building.objects.all().order_by('name'),
        required=False,
        empty_label="Don't Remember",
...
class RegForm(forms.Form):
    ...
    dorm = forms.ModelChoiceField(
        queryset=Building.objects.filter(buildingkind__kind='D').order_by('name'),
        required=True,
    ...
    grad = forms.ChoiceField(
        ...
        choices=(("2011", "2011"), ("2012", "2012"), ("2013", "2013"), ("2014", "2014"), ("2015", "2015"), ("2016", "2016"), ("2017", "2017"), ("", "Not a Student")),
        initial="2011",
        required=False,
    ...
    # TODO: Why is this a DecimalField?
    cell = forms.DecimalField(
        max_digits=10, decimal_places=0,
        required=False,
        help_text="<div class='reminder' >If you want to be able to text message the game's website enter in your phone number here. Include the area code, but do not include hyphens or the leading 1. We will not use this number except in emergencies or in response to texts from you. < /div >"
    )
...
def plot_view(request):
    ...
    days = [d[1] for d in EatForm.DAYS]
    plot = {d:[] for d in days}

    mission = {d: [] for d in days}

    meal = {d: [] for d in days}

    for m in Mission.objects.filter(game=g).filter(Q(show_players="B") | Q(show_players=ui["team"])):
        mission[m.get_day()].append({
            "title": m.get_title(ui["team"]),
            "story": m.get_story(ui["team"]),
        })
...
def forum_new_post_view(request):
    ...
    if request.method == "POST":
        ...
        if post.is_valid():
            ...
            nfp = ForumPost(parent=ForumThread.objects.get(
                id=post.cleaned_data["parent"]),
                contents=post.cleaned_data["contents"],
                creator=ui["player"],
            )
            try:
                nfp.save()
    ...
    if request.method == "POST":
        ...
        if thread.is_valid():
            ...
            ntp = ForumThread(
                ...
                description=thread.cleaned_data["description"],
                visibility=vis,
                game=g,
                creator=ui["player"],
            )
    ...
    if request.method == "POST":
        ...
        if target.exists():
            ...
            if "password" in request.POST:
                ...
                send_mail(
                    "Password Reset",
                    "Your Password for Claremont HvZ has been erased. Please go to http: / / claremonthvz.org / player / passwordreset / " + str(temp) + "/ to enter a new password.",
                    "web@claremonthvz.org",
                    [fix.email],
                    False
                )
                err = "success"

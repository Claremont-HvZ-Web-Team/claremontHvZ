...
class Mission(models.Model):
    ...
    def get_story(self, team):
        ...
        if self.result != "N":
            if team == "H":
                ret += " < b r/>" + self.human.outcome
            else:
                ret += " < b r/>" + self.zombie.outcome
        else:
...
class Attendance is the number of students enrolled in a

class in a building for various times a day during a given game"""
    building = models.ForeignKey(Building)
    game = models.ForeignKey(Game)
    day = models.CharField(max_length=1, choices=DAYS)
